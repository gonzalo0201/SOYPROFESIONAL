...
import { MapPin, ChevronRight, Rocket, Bell } from 'lucide-react';
import { Link } from 'react-router-dom';

export function MapPage() {
    const [storyProfessionalId, setStoryProfessionalId] = useState<number | null>(null);

    const storyProfessional = storyProfessionalId ? MOCK_PROFESSIONALS.find(p => p.id === storyProfessionalId) : null;
    const storyIndex = storyProfessionalId ? MOCK_PROFESSIONALS.findIndex(p => p.id === storyProfessionalId) : -1;
        ...
        <div className="bg-slate-50 min-h-screen pb-24">
            ...
            <div className="bg-slate-900 text-white p-5 pb-8 rounded-b-[2rem] shadow-xl relative z-10 overflow-hidden">
                <div className="relative z-10">
                    ...
                    <p className="text-slate-400 text-sm mt-1">Mostrá lo que sabés hacer. Bahía Blanca te está buscando.</p>
                </div>
            ...
            <div className="-mt-6 relative z-20">
                <div className="bg-white mx-4 rounded-3xl shadow-lg border border-slate-100 overflow-hidden py-1">
                    <StoriesRail onSelectProfessional={(id) => setStoryProfessionalId(id)} />
                </div>
            </div>
            ...
            <div className="p-4 space-y-6">
                        ...
                        <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-[400]">
                            <div className="bg-white/90 backdrop-blur-md px-6 py-3 rounded-2xl shadow-lg flex flex-col items-center gap-1">
                                ...
                                <div className="text-center">
                                    <p className="font-bold text-slate-900 leading-none">Bahía Blanca</p>
                                    <p className="text-[10px] text-slate-500">-38.7183, -62.2663</p>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div className="bg-gradient-to-r from-slate-900 to-slate-800 rounded-3xl p-6 text-white relative overflow-hidden shadow-lg group cursor-pointer">
                    <div className="relative z-10 flex items-center justify-between">
                        <div>
                            ...
                            <h2 className="text-xl font-bold mb-1">Bahía Blanca</h2>
                ...
                <div>
                    ...
                    <div className="space-y-4">
                        {MOCK_PROFESSIONALS.filter(p => p.isBoosted).map(pro => (
                            <ProfessionalCard key={pro.id} professional={pro} />
                        ))}
                        {MOCK_PROFESSIONALS.filter(p => !p.isBoosted).slice(0, 2).map(pro => (
                            <ProfessionalCard key={pro.id} professional={pro} />
                        ))}
                    </div>
                </div>
            ...
            {storyProfessional && (
                <StoryViewer
                    ...
                    onNext={() => {
                        const nextPro = MOCK_PROFESSIONALS[storyIndex + 1];
                        if (nextPro) setStoryProfessionalId(nextPro.id);
                    }}
                    onPrev={() => {
                        const prevPro = MOCK_PROFESSIONALS[storyIndex - 1];
                        if (prevPro) setStoryProfessionalId(prevPro.id);
                    }}
                    hasNext={storyIndex < MOCK_PROFESSIONALS.length - 1}
